<template>
  <div class="space-y-6">
    <div v-for="field in fields" :key="field.key_name">
      ...
      <div v-if="['text', 'number', 'date', 'email'].includes(field.field_type)">
        <input
...
const updateMultiSelect = (key, value) => {
  // Headless UI handles the array toggling for multiple mode automatically?
  // Yes, if `multiple` prop is true, it emits an array.
  const newData = { ...props.modelValue, [key]: value }
  emit('update:modelValue', newData)
}
